...
define(["dojo/_base/declare",
  ...
  "dojo/_base/array",
  "dojo/_base/json",
  "esri/graphic", "esri/symbols/SimpleMarkerSymbol", "esri/layers/GraphicsLayer",
  "esri/tasks/GeometryService", "esri/tasks/ProjectParameters", "esri/Color", "esri/tasks/ProjectParameters",
  "esri/SpatialReference", "esri/InfoTemplate", "dojo/dom", "dojo/on", "esri/geometry/projection", "esri/geometry/Point", "esri/Color",
  "dojo/Deferred",
  "dojo/dom-class",
  "dijit/Viewport",
  "dojo/sniff",
  ...
  function (declare, lang, array, dojoJson, Graphic, SimpleMarkerSymbol, GraphicsLayer, GeometryService, ProjectParameters, Color, ProjectParameters,
    SpatialReference, InfoTemplate, dom, on, projection, Point, Color, Deferred, domClass, Viewport, sniff,
    ...
    return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
      ...
      _addFeatures: function (job, featureCollection) {
        ...
        if (layers.length > 0) {
          ...
          var pt
          var coordArray = layers[0]._graphicsVal
          var cityLayer = new GraphicsLayer()

          cityLayer.on("mouse-over", function (evt) {
          map.setMapCursor("pointer")
        })

        cityLayer.on("mouse-out", function (evt) {
          ...
          for (i = 0; i < coordArray.length; i++) {
            var lat = coordArray[i].attributes.latitude
            var long = coordArray[i].attributes.longitude
            console.log(lat + "," + long)
            var point = new Point({ x: long, y: lat, inSR });
            console.log(point)
            ...
            gsvc.project(prjparams, function (projectedPoints) {
              ...
              graphic.setInfoTemplate(new InfoTemplate(" Coordinates in Nahrwan 1967",
              ...
              load: function (response) {
                ...
                loader._waitForLayer(layer).then(function (lyr) {
                  ...
                  var projOk = (mapSR && outSR) && (mapSR.equals(outSR) ||
                    mapSR.isWebMercator() && outSR.wkid === 27039 ||
                    outSR.isWebMercator() && mapSR.wkid === 27039);
                  if (projOk) {
                    map.addLayer(lyr);
                  } else {
                    new Message({
